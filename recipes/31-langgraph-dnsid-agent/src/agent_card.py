...
def build_agent_card(domain: str, url: str, provider_url: str = "") -> AgentCard:
    ...
    card = AgentCard()
    card.name = "Example DNSid LangGraph Agent"
    card.description = (
        "A LangGraph agent with a DNSid identity: signed tool calls, "
        "caller-gated tools, and DNSid-bound HTTP Message Signatures on ingress."
    )
    card.version = "0.2.0"
    ...
    sk.id = "orders"
    sk.name = "Order desk"
    sk.description = "Prices items and places orders through the organization's tools API."
    sk.tags.extend(["orders", "pricing", "demo"])
    try:
        sk.examples.extend(["What is the price of widgets?", "Please order 3 widgets"])
