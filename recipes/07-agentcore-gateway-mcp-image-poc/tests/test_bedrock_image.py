...
from image_poc.bedrock_image import (
...
def test_build_stability_payload_uses_prompt_style_and_png():
    ...
    assert "a dns image" in payload["prompt"]
...
def test_bedrock_generator_decodes_success_response():
    ...
    client = FakeClient({"images": [encoded], "finish_reasons": [None], "seeds": [1]})
...
def test_bedrock_generator_rejects_finish_reason():
    client = FakeClient({"finish_reasons": ["Filter reason: prompt"]})
    generator = BedrockImageGenerator("model", "region", client=client)

    with pytest.raises(ImageGenerationError):
        generator.generate(validate_generate_payload({"prompt": "hello"}))


def test_bedrock_generator_rejects_malformed_base64():
    client = FakeClient({"images": ["not base64!!!"], "finish_reasons": [None]})
    generator = BedrockImageGenerator("model", "region", client=client)
...
def test_bedrock_generator_rejects_truncated_png():
