"""The agent server: a2a-sdk routes behind the DNSid signature middleware.

Route map of a running agent:
...
The GET well-knowns are what *other* parties fetch to verify us; the POST
route is where we verify *them*. DnsidSignatureMiddleware wraps the whole app
so no unverified POST reaches the a2a-sdk handlers.
"""
...
from middleware import DnsidServerCallContextBuilder, DnsidSignatureMiddleware
