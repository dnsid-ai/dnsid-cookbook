...
Provision a domain identity on the local DNSid testnet — Ed25519 keypair (RFC 8037), registration, signed challenge, published `_dnsid` TXT record, countersigned transparency-log entry — then take it apart by hand: `dig` the record and read every tag, serve and `curl` the JWKS (RFC 7517, at `/.well-known/jwks.json` per RFC 8615), match the served key to the local keypair, check live status. End state: any RFC 9421-aware verifier can authenticate signed messages from your domain. See [recipes/01-publish-jwks/](recipes/01-publish-jwks/).
...
Recipe 6 in Go, end-to-end. A `fake-stripe` ledger exposes `GET /v1/balance` and `POST /v1/balance/credit` behind `dnsid-go` RFC 9421 verification; a worker serves its live JWKS and signs requests with its testnet-provisioned identity. The server verifies the complete DNSid binding, status, transparency-log lifecycle, and HTTP signature before applying a separate writer allowlist. Runs entirely on the DNSid CLI testnet. See [recipes/06b-go-http-stripe/](recipes/06b-go-http-stripe/).
